...
class Renderer {
  ...
  frame(ctx, plan, t, opt = {}) {
    ...
    // In the normal path the frontmost (강조) lyrics are drawn by the redraw after the front 소재 (see below);
    // this flag makes the base pass behave like the foreground branch's lyricLayer:'below' pass, so a frontmost
    // cut — and its transition/morph — is drawn there exactly once, never in both passes.
    let deferFrontmost = false;
    const visible = cut => !opt.noLyrics && (opt.lyricLayer === 'above' ? cut.frontmost : opt.lyricLayer === 'below' || deferFrontmost ? !cut.frontmost : true);
    ...
    // 소재 앞: draw the active 강조(frontmost) lyrics above the front 소재 (the foreground branch does the same
    // at :80). The base pass below defers them via deferFrontmost, and the redraw carries their transition/morph,
    // so each frontmost cut is drawn once, in the order non-frontmost lyrics → front assets → frontmost lyrics.
    // The sub-passes that already handle it (noForeground/noAssets/lyricLayer 'above') and the back layer are skipped.
    const overlayFrontmost = !!assetsOn && layer !== 'back' && !opt.noForeground && !opt.noAssets && opt.lyricLayer !== 'above'
      && !opt.noLyrics && J.lyricCutsAt(plan, tq).some(c => c.frontmost);
    deferFrontmost = overlayFrontmost;
    ...
    if (overlayFrontmost) {
      // noTrans is intentionally not set: the base pass deferred this cut, so the redraw is the single place its
      // transition (and morph) runs. Its backdrop capture at :103-106 keeps that a single application.
      this.frame(ctx, plan, t, Object.assign({}, opt, { noForeground: true, noMedia: true, noAssets: true, transparent: true, preserveCanvas: true, noHud: true, noPost: true, lyricLayer: 'above' }));
    }
  ...
  makeEnv(ctx, plan, cut, sc, o) {
    const area = cut && cut.area && !o.zone && !o.bgOnly && !o.fullFrame ? cut.area : null;
