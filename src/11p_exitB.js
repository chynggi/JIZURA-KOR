/* JIZURA pack: exitB — 39 more exits (퇴장) + 12 more holds (유지 동작)
...
X.peelOff = {
  name: '스티커 떼기', tags: ['pop', 'graphic'], w: 1, ae: 'wipe',
...
X.crumpleOut = {
  name: '둥글려 버리기', tags: ['pop', 'emotional'], w: 0.9, ae: 'shrink', minDur: 0.8,
...
X.tearOut = {
  name: '찢어 버리기', tags: ['emotional', 'graphic'], w: 0.9, ae: 'slice',
...
X.scorchOut = {
  name: '타들어 사라짐', tags: ['emotional', 'glitch'], w: 0.8, ae: 'wipe',
...
X.overexposeOut = {
  name: '하얘짐', tags: ['emotional', 'calm', 'pop'], w: 1, ae: 'blur',
  apply(env, it, p) {
...
X.scanOut = {
  name: '스캔선 지움', tags: ['glitch', 'graphic'], w: 0.9, ae: 'slice',
  apply(env, it, p) {
...
X.stripesOut = {
  name: '줄무늬 지움', tags: ['graphic', 'pop'], w: 0.9, ae: 'wipe',
  apply(env, it, p) {
...
X.halftoneOut = {
  name: '망점으로 사라짐', tags: ['graphic', 'calm'], w: 0.9, ae: 'shrink',
  apply(env, it, p) {
...
X.eraserOut = {
  name: '칠판 지우기', tags: ['editorial', 'calm'], w: 0.9, ae: 'wipe', minDur: 0.9,
  outDur: dur => J.clamp(dur * 0.42, 0.36, 0.85),
  apply(env, it, p) {
...
X.vacuumOut = {
  name: '한 점으로 빨림', tags: ['pop', 'graphic'], w: 0.9, ae: 'shrink',
  apply(env, it, p) {
...
X.sandOut = {
  name: '모래가 되어 날림', tags: ['emotional', 'calm'], w: 1, ae: 'drift',
...
X.shredOut = {
  name: '분쇄기', tags: ['graphic', 'pop'], w: 0.8, ae: 'fall', minDur: 0.9,
...
X.dominoOut = {
  name: '도미노 쓰러짐', tags: ['pop', 'graphic'], w: 0.9, ae: 'fall', minDur: 0.8,
...
X.hingeOut = {
  name: '한쪽 고정 풀림', tags: ['pop', 'editorial'], w: 0.9, ae: 'fall', minDur: 0.9,
...
X.rocketOff = {
  name: '떠오름', tags: ['pop'], w: 0.9, ae: 'drift', minDur: 0.8,
...
X.bounceOff = {
  name: '튕겨 떠남', tags: ['pop'], w: 0.9, ae: 'scatter', minDur: 0.8,
...
X.balloonOff = {
  name: '풍선타고 날음', tags: ['emotional', 'calm', 'pop'], w: 0.9, ae: 'drift', minDur: 0.9,
...
X.deflateOut = {
  name: '바람 빠져 날음', tags: ['pop'], w: 0.8, ae: 'scatter',
  outDur: dur => J.clamp(dur * 0.42, 0.36, 0.85),
  apply(env, it, p) {
...
X.hazeOut = {
  name: '아지랑이 사라짐', tags: ['emotional', 'calm'], w: 0.9, ae: 'blur',
  apply(env, it, p) {
...
X.glassBreak = {
  name: '유리 깨짐', tags: ['graphic', 'pop', 'glitch'], w: 0.8, ae: 'explode',
  outDur: dur => J.clamp(dur * 0.42, 0.36, 0.85),
  apply(env, it, p) {
    const single = isSingle(env, it);
...
X.zipOut = {
  name: '지퍼', tags: ['graphic', 'pop'], w: 0.8, ae: 'wipe',
  apply(env, it, p) {
...
X.clapShut = {
  name: '가운데 닫힘', tags: ['graphic', 'pop'], w: 0.9, ae: 'wipe',
  apply(env, it, p) {
...
X.lampOff = {
  name: '소등', tags: ['glitch', 'emotional'], w: 0.8, ae: 'glitch',
  apply(env, it, p) {
...
X.slotOut = {
  name: '슬롯 회전', tags: ['glitch', 'pop'], w: 0.8, ae: 'glitch',
...
X.clockOut = {
  name: '시계 와이프', tags: ['graphic', 'editorial'], w: 0.9, ae: 'wipe',
  apply(env, it, p) {
...
X.matrixOut = {
  name: '디지털 빗줄기', tags: ['glitch'], w: 0.8, ae: 'glitch',
...
X.tornadoOut = {
  name: '회오리바람', tags: ['pop'], w: 0.8, ae: 'scatter', minDur: 0.8,
  outDur: dur => J.clamp(dur * 0.45, 0.38, 0.9),
  apply(env, it, p) {
...
X.rollUpOut = {
  name: '감아 들임', tags: ['graphic', 'editorial'], w: 0.8, ae: 'wipe',
  outDur: dur => J.clamp(dur * 0.4, 0.32, 0.8),
  apply(env, it, p) {
...
X.snakeOut = {
  name: '줄지어 떠남', tags: ['calm', 'pop'], w: 0.9, ae: 'stretch', minDur: 0.8,
  outDur: dur => J.clamp(dur * 0.45, 0.38, 0.9),
  apply(env, it, p) {
...
X.flutterOut = {
  name: '나풀나풀 떨어짐', tags: ['emotional', 'calm'], w: 1, ae: 'fall', minDur: 0.8,
  outDur: dur => J.clamp(dur * 0.48, 0.42, 1.0),
  apply(env, it, p) {
...
X.rollOff = {
  name: '구르며 떠남', tags: ['pop'], w: 0.8, ae: 'scatter', minDur: 0.8,
  outDur: dur => J.clamp(dur * 0.45, 0.38, 0.9),
  apply(env, it, p) {
...
X.fanClose = {
  name: '부채 접기', tags: ['graphic', 'editorial', 'emotional'], w: 0.9, ae: 'shrink', minDur: 0.8,
...
X.rgbSplitOut = {
  name: '색 분해', tags: ['glitch', 'pop'], w: 0.9, ae: 'glitch',
  apply(env, it, p) {
...
X.shockOut = {
  name: '충격파', tags: ['pop', 'graphic'], w: 0.9, ae: 'explode',
  apply(env, it, p) {
...
X.floodOut = {
  name: '물에 잠김', tags: ['emotional', 'calm'], w: 0.9, ae: 'wipe',
  outDur: dur => J.clamp(dur * 0.4, 0.32, 0.8),
  apply(env, it, p) {
...
X.slashOut = {
  name: '한 칼 두 동강', tags: ['graphic', 'emotional', 'pop'], w: 1, ae: 'slice',
  outDur: dur => J.clamp(dur * 0.4, 0.32, 0.8),
  apply(env, it, p) {
...
let MOS = null;
X.mosaicOut = {
  name: '모자이크', tags: ['glitch', 'graphic'], w: 0.9, ae: 'glitch',
  apply(env, it, p) {
...
X.scribbleOut = {
  name: '구겨 지움', tags: ['editorial', 'emotional', 'pop'], w: 0.8, ae: 'wipe',
  outDur: dur => J.clamp(dur * 0.4, 0.32, 0.8),
  apply(env, it, p) {
...
X.candleOut = {
  name: '후 불어 지움', tags: ['emotional', 'calm'], w: 0.9, ae: 'drift',
  outDur: dur => J.clamp(dur * 0.45, 0.38, 0.9),
  apply(env, it, p) {
...
H.glowFlicker = {
  name: '불빛 흔들림', tags: ['calm', 'emotional'], w: 0.5, ae: 'breathe',
  apply(env, it, amt) {
...
H.windGust = {
  name: '돌풍', tags: ['pop', 'emotional'], w: 0.5, ae: 'wave',
  apply(env, it, amt) {
    const k = amt * motionK(env); if (k < 0.01) return;
...
H.dangle = {
  name: '매달림', tags: ['calm', 'emotional'], w: 0.5, ae: 'wave',
  apply(env, it, amt) {
    const k = amt * motionK(env); if (k < 0.01) return;
...
H.eqBounce = {
  name: '음압에 늘어남', tags: ['pop', 'graphic'], w: 0.5, ae: 'breathe',
  apply(env, it, amt) {
    const k = amt * motionK(env); if (k < 0.01) return;
...
H.flashBox = {
  name: '박자마다 반전', tags: ['pop', 'graphic', 'glitch'], w: 0.4, ae: 'glitchtick',
  apply(env, it, amt) {
...
H.glintSweep = {
  name: '광택', tags: ['graphic', 'calm'], w: 0.5, ae: 'still',
  apply(env, it, amt) {
...
H.flipSwap = {
  name: '가끔 뒤집힘', tags: ['glitch', 'pop'], w: 0.4, ae: 'glitchtick',
  apply(env, it, amt) {
...
H.shadowSway = {
  name: '그림자 흔들림', tags: ['calm', 'emotional'], w: 0.5, ae: 'drift',
  apply(env, it, amt) {
...
H.magnetJiggle = {
  name: '자력', tags: ['pop', 'glitch'], w: 0.4, ae: 'jitter',
  apply(env, it, amt) {
    const k = amt * motionK(env); if (k < 0.01) return;
...
H.typeRattle = {
  name: '타이핑 떨림', tags: ['editorial', 'glitch'], w: 0.5, ae: 'jitter',
  apply(env, it, amt) {
    const k = amt * motionK(env); if (k < 0.01) return;
...
H.focusRack = {
  name: '초점 훑기', tags: ['calm', 'emotional'], w: 0.5, ae: 'breathe',
  apply(env, it, amt) {
    const k = amt * motionK(env); if (k < 0.01) return;
...
H.pluckString = {
  name: '현의 떨림', tags: ['pop', 'emotional'], w: 0.4, ae: 'wave',
  apply(env, it, amt) {
    const k = amt * motionK(env); if (k < 0.01) return;
    let tau;
