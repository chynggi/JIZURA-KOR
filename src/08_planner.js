...
J.parseLyrics = (raw) => {
  ...
  for (let src of String(raw || '').replace(/\r/g, '').split('\n')) {
    ...
    const base = { text: s, note, impact, emph, manual, gapBefore: pendingGap, breakBefore: pendingBreak };
...
J.plan = (project, audio) => {
  ...
  parsed.lines.forEach((ln, li) => {
    ...
    const recap = !mk && nC > groups.length && groups.length >= 2;
    ...
    units.forEach((u, k) => {
      ...
      const treat = ov.treat && J.TREAT[ov.treat] ? ov.treat : pickTreat(rng, st, en, fx, LD, emph, history, !!mk);
