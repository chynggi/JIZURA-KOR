...
/* ================= lens / optics ================= */
fx('radialChroma', { name: '방사 색수차', tags: ['glitch', 'emotional', 'pop'], w: 0.9, dur: 4, amp: 1, mid: true, scratch: true, ae: 'chroma',
  draw(ctx, ev, k, I) {
...
fx('bloomFlash', { name: '블룸', tags: ['pop', 'emotional', 'calm'], w: 1, dur: 6, amp: 1, mid: true, scratch: true, ae: 'flash',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('bulge', { name: '어안', tags: ['pop', 'graphic', 'glitch'], w: 0.8, dur: 5, amp: 1, mid: true, scratch: true, ae: 'zoom',
  draw(ctx, ev, k, I) {
...
fx('pixelSort', { name: '픽셀 정렬', tags: ['glitch'], w: 0.8, dur: 3, amp: 1, glitchy: true, mid: true, scratch: true, ae: 'slice',
  draw(ctx, ev, k, I) {
    const { cw, ch, S } = I; if (!S) return;
...
fx('interlace', { name: '인터레이스', tags: ['glitch', 'emotional'], w: 0.7, dur: 3, amp: 1, glitchy: true, mid: true, scratch: true, ae: 'slice',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('macroBlock', { name: '블록 노이즈', tags: ['glitch'], w: 0.8, dur: 3, amp: 1, glitchy: true, mid: true, scratch: true, ae: 'block',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('halftone', { name: '망점', tags: ['pop', 'graphic', 'editorial'], w: 0.9, dur: 5, amp: 1, mid: true, ae: 'mosaic',
  draw(ctx, ev, k, I) {
    const { cw, ch, sc } = I;
...
fx('duotone', { name: '더블톤', tags: ['pop', 'emotional', 'graphic'], w: 0.8, dur: 4, amp: 1, mid: true, ae: 'chroma',
  draw(ctx, ev, k, I) {
...
fx('ditherBit', { name: '1비트 디더', tags: ['glitch', 'graphic', 'pop'], w: 0.7, dur: 3, amp: 1, mid: true, scratch: true, ae: 'mosaic',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
/* ================= frame motion ================= */
fx('rotateSnap', { name: '기울임 스냅', tags: ['pop', 'graphic', 'glitch'], w: 0.9, dur: 5, amp: 1, mid: true, scratch: true, ae: 'shake',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('echoFrames', { name: '잔상 에코', tags: ['emotional', 'pop', 'glitch'], w: 0.9, dur: 6, amp: 1, mid: true, scratch: true, ae: 'zoom',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('kaleido', { name: '만화경', tags: ['pop', 'graphic', 'emotional'], w: 0.6, dur: 4, amp: 1, mid: true, scratch: true, ae: 'block',
  draw(ctx, ev, k, I) {
    const { cw, ch, S } = I; if (!S) return;
...
/* ================= inversion / light ================= */
fx('bandInvert', { name: '띠 반전', tags: ['glitch', 'graphic'], w: 0.7, dur: 3, amp: 1, glitchy: true, mid: true, ae: 'invert',
  draw(ctx, ev, k, I) {
...
fx('lightRays', { name: '빛살', tags: ['emotional', 'pop', 'calm'], w: 0.9, dur: 8, amp: 1, mid: true, ae: 'flash',
  draw(ctx, ev, k, I) {
...
fx('anamorphic', { name: '아나모 플레어', tags: ['emotional', 'pop', 'calm'], w: 0.8, dur: 7, amp: 1, mid: true, ae: 'flash',
  draw(ctx, ev, k, I) {
    const { cw, ch, sc } = I, s = evS(ev), dk = isDark(sc.bg);
...
fx('heartbeat', { name: '박동', tags: ['emotional', 'calm'], w: 0.8, dur: 10, amp: 1, mid: true, scratch: true, ae: 'zoom',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('tvStatic', { name: 'TV 잡음', tags: ['glitch', 'emotional'], w: 0.6, dur: 5, pre: 2, amp: 1, glitchy: true, ae: 'block',
  draw(ctx, ev, k, I) {
...
fx('dustScratches', { name: '필름 긁힘', tags: ['emotional', 'calm', 'editorial'], w: 0.8, dur: 8, amp: 1, mid: true,
  draw(ctx, ev, k, I) {
...
fx('filmAdvance', { name: '필름 감기', tags: ['emotional', 'editorial', 'calm'], w: 0.6, dur: 9, pre: 4, amp: 1, scratch: true, ae: 'slice',
  draw(ctx, ev, k, I) {
    const { cw, ch, S } = I; if (!S) return;
...
fx('perspectiveTilt', { name: '원근 흔들림', tags: ['pop', 'graphic', 'emotional'], w: 0.8, dur: 8, amp: 1, mid: true, scratch: true, ae: 'shake',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('ripple', { name: '잔물결', tags: ['emotional', 'calm', 'pop'], w: 0.8, dur: 10, amp: 1, mid: true, scratch: true, ae: 'zoom',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
// 집중선: thin wedges converging on the centre, redrawn every frame like hand-drawn animation
fx('focusLines', { name: '집중선', tags: ['pop', 'graphic', 'emotional'], w: 0.9, dur: 6, amp: 1, mid: true, ae: 'zoom',
  draw(ctx, ev, k, I) {
...
// 속도선: tapered streaks racing across the frame (clear of the centre band)
fx('speedLines', { name: '속도선', tags: ['pop', 'graphic'], w: 0.8, dur: 6, amp: 1, mid: true,
  draw(ctx, ev, k, I) {
...
// 반짝: 8-point glints popping in sequence around the lyric band
fx('starGlint', { name: '반짝', tags: ['pop', 'emotional'], w: 0.8, dur: 9, amp: 1, mid: true, ae: 'flash',
  draw(ctx, ev, k, I) {
...
fx('colorBars', { name: '컬러 바', tags: ['pop', 'graphic', 'glitch'], w: 0.8, dur: 6, amp: 1, mid: true, ae: 'slice',
  draw(ctx, ev, k, I) {
...
// three hard zoom steps (다다단), each with its own slight focus shift, then snap back
fx('zoomStutter', { name: '줌 연타', tags: ['pop', 'graphic', 'glitch'], w: 0.8, dur: 6, amp: 1, mid: true, scratch: true, ae: 'zoom',
  draw(ctx, ev, k, I) {
    const { cw, ch, S } = I; if (!S) return;
...
fx('negativeRing', { name: '반전 링', tags: ['graphic', 'pop', 'glitch'], w: 0.8, dur: 6, amp: 1, mid: true, ae: 'invert',
  draw(ctx, ev, k, I) {
...
fx('edgeDetect', { name: '외곽선 추출', tags: ['graphic', 'glitch', 'editorial'], w: 0.7, dur: 4, amp: 1, mid: true, scratch: true, ae: 'invert',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('shatter', { name: '유리 깨짐', tags: ['glitch', 'pop', 'emotional'], w: 0.5, dur: 8, pre: 1, amp: 1, scratch: true, ae: 'block',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('defocus', { name: '초점 흔들림', tags: ['emotional', 'calm', 'editorial'], w: 0.9, dur: 8, pre: 4, amp: 1, mid: true, scratch: true, ae: 'zoom',
  draw(ctx, ev, k, I) {
    const { cw, ch, S } = I; if (!S) return;
...
fx('snapshot', { name: '셔터', tags: ['pop', 'emotional', 'editorial'], w: 0.6, dur: 10, amp: 1, mid: true, scratch: true, ae: 'flash',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('squash', { name: '신축', tags: ['pop', 'graphic'], w: 0.8, dur: 6, amp: 1, mid: true, scratch: true, ae: 'zoom',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('scanBar', { name: '스캔', tags: ['graphic', 'editorial', 'glitch'], w: 0.7, dur: 8, amp: 1, mid: true, scratch: true, ae: 'flash',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('loopScroll', { name: '가로 루프', tags: ['pop', 'graphic', 'glitch'], w: 0.7, dur: 6, pre: 3, amp: 1, scratch: true, ae: 'slice',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
