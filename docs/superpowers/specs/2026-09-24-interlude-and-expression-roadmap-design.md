...
- 카메라 `parallaxPan`: 가로로 천천히 이동하면서 약한 원근 기울기(skx) — 레이아웃 층과 별개로 모든 컷에 사용 가능. 
- 등장 `penWrite`: 글자별 외곽선을 순서대로 그리는 동안 펜촉 점이 획 끝을 따라가고, 획이 끝난 글자는 채워짐(`dash` 진행 + 펜촉 점 + 순차). ae: `type`.
- 장식 `markerSwipe`: 가사 bbox 아래를 형광펜이 손으로 긋듯 흔들리며 지나감(두께 변화·끝 번짐). ae: `bars`.
