...
var JZCEP = (function () {
    ...
    function step(ms) {
        if (!job) return fail('생성 중인 컴포가 없습니다');
        var C = core(), err = null;
        app.beginUndoGroup('JIZURA');
    ...
    return {
        ...
        buildFromFile: function (path, audioId) { var s = readTemp(path); return s == null ? fail('구성 데이터 임시 파일을 찾을 수 없습니다') : build(s, audioId); },
        buildFromString: function (enc, audioId) { return build(decodeURIComponent(enc), audioId); },
        startFromFile: function (path, audioId, light) { var s = readTemp(path); return s == null ? fail('구성 데이터 임시 파일을 찾을 수 없습니다') : start(s, audioId, light); },
