...
- 通常の機能はインストール不要。歌詞・曲・書き出しはブラウザ内で処理されます（外部から読み込むのは Google Fonts のフォントだけです）。
...
- MP4（曲入り）・連番PNG・透過PNG で書き出し。背景をグリーンバック / ブラックバックにした合成用出力にも対応します。After Effects で編集できる構成データ（JSON）も書き出せます。
- 画像・動画を背景または前景としてアップロードできます。前景は歌詞の上に表示され、動画にはカットごとのクロマキー合成を設定できます。
- 歌詞の行ごとに表示エリアを指定できます。「行とカット」の小さなサムネイルから編集します。

タイムライン上のカット開始境界をドラッグすると開始時刻を調整できます。歌詞の行内カット、前景、背景にも対応します。境界以外をドラッグすると従来どおり再生位置を移動します。調整した時刻はプロジェクトに保存され、「手動タイミングを消す」で解除できます。

### 歌詞の表示エリア

歌詞タブの「行とカット」にある表示エリアのサムネイルをクリックすると編集モードになります。プレビュー上をドラッグして長方形を描き、「この行だけに適用」または「これ以降全てに適用」を選びます。「全域へリセット」を押した後に適用すると、対象行の表示エリア指定を解除します。「キャンセル」では変更しません。指定した行の歌詞カットと装飾は長方形の寸法に合わせて構成され、文字の縦横比を保ちます。背景・前景素材の表示エリアは変わりません。設定はプロジェクトとともに保存され、「おまかせ」による再構成後も維持されます。

### 歌詞を重ねる・カット内で改行する・自動配置する

`{` と `}` で囲んだ行は、各カットが始まるたびに上へ重なり、グループの最後の歌詞カットが消えるまで継続表示します。タイムラインの開始時刻と境界リンクはそのまま利用できます。
...
同じ入力行に `\n` を書くと、1カット内に横書きの改行を入れられます。`/` と併用すると、区切った各カット内で改行できます。`\\n` は文字としての `\n`、`\{`・`\}`・`\~` は制御文字そのものを表示します。

「詳細 → 手法」の「配置・サイズにも自動で変化をつける」をオンにすると、歌詞カットごとの表示エリアにもバリエーションが付きます。初期状態はオフで全画面を使用します。手動指定したエリアとロックは維持され、表示エリア編集の「自動配置に戻す」で手動指定を解除できます。「全域へリセット」は全画面の手動指定として保存します。

「前景との重なりを避ける」は、自動配置時に、各カット本来の時間帯に表示される前景素材の配置・サイズ・回転を考慮して空いた領域を選びます。`{}` で継続表示する歌詞も、通常カットと同じ基準で個別に表示エリアを割り当てます。空きがない場合は重なりの少ない位置を選びます。`*強調*` は自動エリアを大きくし、「最前に表示」をオンにして前景回避の対象外にします。最前面表示はカットのチェックやタイムラインのアイコンで変更できます。`~抑制~` はエリアや文字を小さくし、移動・回転・カメラや演出の動きを控えめにします。

### 歌詞カットの合成方法・不透明度

「行とカット」の各カットで、合成方法（通常・乗算・スクリーン・オーバーレイ）と不透明度（0〜100%）を設定できます。初期値は通常・100%です。`{}` で重ねた歌詞も、それぞれの設定で合成します。

「詳細 → 手法」の「合成方法もランダム」「歌詞不透明度もランダム」は、それぞれ独立してオンにできます。不透明度のランダムをオンにすると、最低・最大不透明度を指定できます（初期範囲0〜100%）。`*強調*` は範囲の上側3分の1、`~抑制~` は下側3分の1から選びます。シャッフルや再抽選で値が変わり、同じシードでは同じ値になります。

手動指定とロックを優先します。カットの「自動に戻す」で手動指定を解除できます。ランダム設定がオフの場合、自動の値は通常・100%になります。旧プロジェクトの一括設定は、読み込み時に既存カットの個別設定へ引き継ぎます。

### 画像・動画を背景・前景に使う

左側の「前景」「歌詞」「背景」タブは歌詞を初期表示します。「背景」「前景」タブからそれぞれにファイルを追加します。素材管理エリアへのドラッグ＆ドロップにも対応します。背景は歌詞の下、前景は歌詞の上に表示されます。素材一覧にはサムネイルとファイル名が表示され、「行とカット」のファイル名をクリックするとカットごとの素材を選べます。カットの先頭・間・末尾の「カットを追加」で空のカットを挿入でき、初期値は「画像無し」です。既存カットの素材と開始時刻は維持されます。歌詞タブでは歌詞、前景タブでは前景素材の合成方法と不透明度を調整できます。

従来の自動カットでは「ランダム順」と「ループ表示」も利用できます。手動で追加したカットの素材は空のため、ファイル名をクリックして指定してください。ループ表示中に「タップで同期」を始めた場合も、タップごとに「画像無し」のカットを追加します。

「行とカット」で素材ごとの開始秒と、画像・動画に使える表示方法・登場・保持・退場・加工・カット間のつなぎを設定できます。動画のカットには初期状態でオンの「動画をループ再生」と「クロマキー合成」があり、後者は指定色を透明にします。表示方法は「全画面」と「全体を表示」で、元画像の縦横比を保ちます。既存プロジェクトの「引き伸ばす」は「全画面」として扱います。「タップで同期」は開いているタブを対象にします。プレビュー下のタイムラインは上から前景・歌詞・背景です。動画の音声は使用せず、曲として読み込んだ音声だけを書き出します。

画像・動画の角度は前景・背景の「配置・サイズを編集」で−180〜180°の範囲から指定できます。素材の枠の周囲をドラッグして回転することもできます。ズーム率・ズーム対象の設定は使用しません。

前景・背景の「行とカット」にある「配置・サイズを編集」を押すと、プレビュー上に素材が表示されます。素材をドラッグして移動し、四隅をドラッグしてサイズを変更できます。アスペクト比は初期状態で固定され、チェックを外すと幅と高さを別々に変更できます。幅・高さは画面比0.5〜400％を入力でき、画面より大きく配置することもできます。「このカットだけに適用」または「これ以降全てに適用」で保存し、「自動配置に戻す」で解除できます。編集中は動きの演出を一時停止して配置を確認します。新しいカットの初期設定は「全体を表示」「即時」「静止」「即時」「なし」で、つなぎは「おまかせ」です。

「保存」は設定・歌詞と、登録済みの画像・動画・曲・取り込んだフォントファイルをまとめた `.jizuraichi` ファイルを保存します。「開く」で別のブラウザやPCでも素材ごと復元できます。PCのインストール済みフォントを名前だけで指定した場合、そのフォントのインストールは別途必要です。読み込みダイアログは `.jizuraichi` ファイルを対象にします。素材不足時は不完全なプロジェクトファイルを保存せず、エラーを表示します。ブラウザ内の自動保存は IndexedDB と設定データを使い、曲も復元します。AE用JSONは引き続き構成データのみです。

「新規」で画面比を選び、「作成」を押すと歌詞・素材・曲・タイトルが空のプロジェクトになります。設定は初期値に戻り、Undoと案の履歴もリセットされます。現在の編集を残す場合は先に「保存」してください。
...
曲アップロード箇所の下の「テーマ設定」で複数選択し、「適用」で保存します。曲の下に選択したテーマのラベルが表示されます。

- 曲ジャンル：ポップ、バラード、ロック、ダンス・EDM、ヒップホップ、ジャズ、アコースティック
- テイスト：クール、可愛い、上品、幻想的、レトロ

「おまかせ」では選択テーマの中から毎回ひとつを選び、テーマ専用のスタイル・配色・文字演出と、前景・背景のモーション・登場・退場の候補から構成します。テーマの外から候補を混ぜません。複数選択はテーマごとの構成を交互に試す使い方です。未選択では従来の制限なしになります。テーマを選んだだけでは現在の演出は変わらず、次のおまかせから適用します。手動の素材演出・配置、ロック済みのカットは従来どおり優先します。追加分・和風の許可設定を尊重します。テーマはプロジェクトファイル、ブラウザ保存、Undo/Redoに対応します。

ローカルで確認するには `python preview_server.py` を実行して `http://127.0.0.1:8765/` を開きます。テーマ選定はブラウザ内で完結し、APIキーは不要です。
...
前景・背景の素材管理で「見切れている辺（自動配置）」の左・右・上・下を選べます。同じ素材を使うカットの自動配置では、指定した各辺から素材の幅・高さの約20％が画面外になるよう配置し、両側の指定で必要な場合は縦横比を保って拡大します。手動配置・ロックは優先されます。設定は素材単位で保存されます。これは自動で決める表示範囲への指定で、演出による移動・回転を制限するものではありません。

詳細 → 手法の「重なり回避の強度」は0〜1（初期値1）です。1は前景の表示領域を避け、値を下げると前景の周辺に重なる配置を許容します。0は回避なしです。自動配置と「前景との重なりを避ける」がオンのときに適用します。強調カット・手動指定・ロックは従来どおり優先します。前景が全域を覆うなど空き領域がない場合は、最も重なりの少ない配置を使います。

### 右上のメニュー

「プロジェクト」に新規・開く・保存をまとめています。「出力」からMP4、連番PNG（ZIP）、透過PNG（ZIP・背景なし）、AE用構成データを書き出せます。MP4・PNGは書き出し設定モーダルを開きます。設定は詳細 → 書き出しと共通で、MP4では画質・音声、PNGでは画像用の設定を表示します。モーダル内で書き出しを開始し、進捗を確認・中止できます。

書き出しモーダルの「ファイル名」で、MP4・連番PNG ZIP・透過PNG ZIPの保存名を指定できます。プロジェクト保存とAE用書き出しでも名前入力ダイアログを表示します。拡張子は自動付加し、入力済みなら重複しません。空欄では曲名から自動生成し、ファイル名に使えない文字は置き換えます。
