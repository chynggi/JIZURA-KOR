...
画像・動画の各カットにはズーム率（100〜300％）とズーム対象（画面を9分割した位置）も指定できます。どちらも空欄・「おまかせ」で、そのカットのシードに基づく自動選択になります。

前景の「行とカット」にある「配置・サイズを編集」を押すと、プレビュー上に素材が表示されます。素材をドラッグして移動し、四隅をドラッグして縦横比を保ちながらサイズを変更できます。「このカットだけに適用」または「これ以降全てに適用」で保存し、「自動配置に戻す」で解除できます。編集中は動きの演出を一時停止して配置を確認します。
