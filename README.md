...
「前景との重なりを避ける」は、自動配置時に、表示期間中の前景素材の配置・サイズ・回転を考慮して空いた領域を選びます。空きがない場合は重なりの少ない位置を選びます。`*強調*` は自動エリアを大きくし、「最前に表示」をオンにして前景回避の対象外にします。最前面表示はカットのチェックやタイムラインのアイコンで変更できます。`~抑制~` はエリアや文字を小さくし、移動・回転・カメラや演出の動きを控えめにします。
