...
左側の「背景」「前景」タブから、それぞれにファイルを追加します。背景は歌詞の下、前景は歌詞の上に表示されます。両タブで同じ素材管理・カット設定を使え、サムネイルをドラッグして順番を変えられます。「ランダム順」をオンにすると、カット開始時に決定済みのランダム順で表示します。背景では歌詞、前景では前景素材の合成方法と不透明度を調整できます。
