...
素材ファイルはこのブラウザの IndexedDB に保存します。プロジェクト JSON には素材の名前や設定だけが入り、画像・動画の本体は含まれません。別のブラウザや端末でプロジェクトを開くときは、同じ名前とサイズの素材を該当するタブで再度追加してください。AE 用 JSON は素材ファイルを含まないため、背景・前景はブラウザ版で書き出してください。
