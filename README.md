...
左側の「前景」「歌詞」「背景」タブは歌詞を初期表示します。「背景」「前景」タブからそれぞれにファイルを追加します。背景は歌詞の下、前景は歌詞の上に表示されます。両素材タブで同じ素材管理・カット設定を使え、サムネイルをドラッグして順番を変えられます。「ランダム順」をオンにすると、カット開始時に決定済みのランダム順で表示します。歌詞タブでは歌詞、前景タブでは前景素材の合成方法と不透明度を調整できます。

「ループ表示」をオンにすると、カット数を自由に設定できます。素材の順番（ランダム順ならその抽選順）を繰り返し、各カットのタイミングと演出は個別に設定できます。オフでは素材1つにつき1カットです。ループ表示中に「タップで同期」を始めると、タップするたびに次のカットが1つ追加されます。カット数を事前に決める必要はなく、「終了する」か曲の終わりまで続けられます。
