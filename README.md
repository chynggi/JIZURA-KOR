...
歌詞タブの「行とカット」にある表示エリアのサムネイルをクリックすると編集モードになります。プレビュー上をドラッグして長方形を描き、「この行だけに適用」または「これ以降全てに適用」を選びます。「全域へリセット」を押した後に適用すると、対象行の表示エリア指定を解除します。「キャンセル」では変更しません。指定した行の歌詞カットと装飾は長方形の寸法に合わせて構成され、文字の縦横比を保ちます。背景・前景素材の表示エリアは変わりません。設定はプロジェクトとともに保存され、「おまかせ」や Jev による再構成後も維持されます。
...
「保存」は設定・歌詞と、登録済みの画像・動画・曲・取り込んだフォントファイルをまとめた `.jizura` ファイルを保存します。「開く」で別のブラウザやPCでも素材ごと復元できます。PCのインストール済みフォントを名前だけで指定した場合、そのフォントのインストールは別途必要です。従来の `.jizura.json` も開けますが、その形式には素材本体が含まれないため、素材が見つからなければ再追加してください。素材不足時は不完全なプロジェクトファイルを保存せず、エラーを表示します。ブラウザ内の自動保存は IndexedDB と設定データを使い、曲も復元します。AE用JSONは引き続き構成データのみです。
...
[Jev 機能（β）の使い方](docs/JEV_GUIDE.md)

歌詞に合う雰囲気・スタイルと、行ごとのレイアウト・登場・退場を Jev で選べます。歌詞欄の下に自然言語の追加指示を任意で入力できます。選定には「追加分の演出」「和風の演出」の切り替えも反映されます。

TypeSafe の API キーを取得し、このリポジトリのルートでローカルサーバーを起動してください（Python 3 の標準ライブラリのみ）。
...
- 通常の機能では歌詞や曲はブラウザ内で処理されます。Jev を使う場合は歌詞と任意の追加指示を Jev API に送信します。曲の音声ファイルは送信しません。
