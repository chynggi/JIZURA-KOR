...
- 歌詞欄の下にある「Jevへの追加指示（任意）」で、目指す雰囲気やサビの演出などを自然言語で指定できます。追加指示は Jev で作るときだけ使い、プロジェクトにも保存されます。
...
キーはサーバー環境変数から API に送られ、ブラウザ・プロジェクトファイルには保存されません。Jev に送るのは歌詞・曲名・アーティスト名・任意の追加指示と選択肢です。音声ファイルは送信しません。Jev API が失敗した場合は現在の案を維持します。従来の「おまかせ」は引き続きオフラインで動きます。ローカルサーバーはこのフォークの Pages とローカル画面からの API 呼び出しだけを許可します。
