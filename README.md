...
ブラウザで <http://127.0.0.1:8765/> を開いて歌詞を入力し、「Jevで作る」を押します。キーはサーバー環境変数から API に送られ、ブラウザ・プロジェクトファイルには保存されません。Jev に送るのは歌詞・曲名・アーティスト名と選択肢です。音声ファイルは送信しません。Jev API が失敗した場合は現在の案を維持します。従来の「おまかせ」は引き続きオフラインで動きます。

GitHub Pages は静的ホスティングのため、Pages 上では Jev 用の `/api/jev` を提供できません。Jev 連携はこのローカルサーバーで使用してください。
