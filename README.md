...
- 画像・動画をアップロードして背景カットにできます。素材ごとの表示方法・登場・保持・退場・加工と、歌詞の合成方法・不透明度を指定できます。

### 画像・動画を背景にする

左側の「画像・動画」タブからファイルを追加します。サムネイルをドラッグして順番を変えられます。「ランダム順」をオンにすると、カット開始時に決定済みのランダム順で表示します。歌詞の合成方法は通常・乗算・スクリーンから選び、不透明度も調整できます。
...
「行とカット」で素材ごとの開始秒と、画像・動画に使える表示方法・登場・保持・退場・加工・カット間のつなぎを設定できます。動画のカットには「動画をループ再生」もあり、オンにすると素材の終端から先頭に戻り、カットの終わりまで再生します。表示方法は「全画面」と「全体を表示」で、元画像の縦横比を保ちます。既存プロジェクトの「引き伸ばす」は「全画面」として扱います。「タップで同期」は開いているタブを対象にします。プレビュー下の2本目のタイムラインは画像・動画のカットです。動画の音声は使用せず、曲として読み込んだ音声だけを書き出します。

画像・動画の各カットにはズーム率（100〜300％）とズーム対象（画面を9分割した位置）も指定できます。どちらも空欄・「おまかせ」で、そのカットのシードに基づく自動選択になります。

素材ファイルはこのブラウザの IndexedDB に保存します。プロジェクト JSON には素材の名前や設定だけが入り、画像・動画の本体は含まれません。別のブラウザや端末でプロジェクトを開くときは、同じ名前とサイズの素材を再度追加してください。AE 用 JSON は素材ファイルを含まないため、画像・動画の背景はブラウザ版で書き出してください。
