...
「行とカット」で素材ごとの開始秒と、画像・動画に使える表示方法・登場・保持・退場・加工・カット間のつなぎを設定できます。表示方法は「全画面」と「全体を表示」で、元画像の縦横比を保ちます。既存プロジェクトの「引き伸ばす」は「全画面」として扱います。「タップで同期」は開いているタブを対象にします。プレビュー下の2本目のタイムラインは画像・動画のカットです。動画の音声は使用せず、曲として読み込んだ音声だけを書き出します。
