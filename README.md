...
前景・背景の素材管理で「見切れている辺（自動配置）」の左・右・上・下を選べます。同じ素材を使うカットの自動配置では、指定した辺が画面外になるよう配置し、両側の指定で必要な場合は縦横比を保って拡大します。手動配置・ロックは優先されます。設定は素材単位で保存されます。これは自動で決める表示範囲への指定で、演出による移動・回転を制限するものではありません。
