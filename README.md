...
歌詞タブの「行とカット」にある表示エリアのサムネイルをクリックすると編集モードになります。プレビュー上をドラッグして長方形を描き、「この行だけに適用」または「これ以降全てに適用」を選びます。「キャンセル」では変更しません。指定した行の歌詞カットと装飾は長方形の寸法に合わせて構成され、文字の縦横比を保ちます。背景・前景素材の表示エリアは変わりません。設定はプロジェクトとともに保存され、「おまかせ」や Jev による再構成後も維持されます。
...
「行とカット」で素材ごとの開始秒と、画像・動画に使える表示方法・登場・保持・退場・加工・カット間のつなぎを設定できます。動画のカットには「動画をループ再生」と「クロマキー合成」があり、後者は指定色を透明にします。表示方法は「全画面」と「全体を表示」で、元画像の縦横比を保ちます。既存プロジェクトの「引き伸ばす」は「全画面」として扱います。「タップで同期」は開いているタブを対象にします。プレビュー下のタイムラインは上から前景・歌詞・背景です。動画の音声は使用せず、曲として読み込んだ音声だけを書き出します。

動画の各カットにはズーム率（100〜300％）とズーム対象（画面を9分割した位置）も指定できます。未指定の初期値は100％・中央で、カットを再抽選した場合はシードに基づき選択されます。

画像の角度は前景・背景の「配置・サイズを編集」で−180〜180°の範囲から指定できます。画像のズーム率・ズーム対象は使用しません。
