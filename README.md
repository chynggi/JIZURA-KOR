...
歌詞タブの「行とカット」にある表示エリアのサムネイルをクリックすると編集モードになります。プレビュー上をドラッグして長方形を描き、「この行だけに適用」または「これ以降全てに適用」を選びます。「キャンセル」では変更しません。指定した行の歌詞カットと装飾は長方形内に収まり、背景・前景素材の表示エリアは変わりません。設定はプロジェクトとともに保存されます。
