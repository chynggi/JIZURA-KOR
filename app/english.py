...
BODY = {
    ...
    'カット挿入': 'Insert cuts', 'ランダム順で挿入': 'Insert in random order', 'ループ': 'Loop',
...
FORK_BODY = {
    ...
    '歌詞不透明度もランダム': 'Randomize lyric opacity',
    '通常・乗算・スクリーン・オーバーレイからカットごとに選びます。': 'Choose Normal, Multiply, Screen or Overlay for each cut.',
    '指定範囲内で、*強調*は高め、~抑制~は低めにします。手動指定とロックを優先します。': 'Within the range, *emphasis* is more opaque and ~suppression~ is less opaque. Manual values and locks take priority.',
    '最低不透明度（％）': 'Minimum opacity (%)',
    '最大不透明度（％）': 'Maximum opacity (%)',
