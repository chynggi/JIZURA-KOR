  ...
  try {
    for(const locale of ['', 'en/']) {
      ...
      for(const layer of ['foreground','media']) {
        ...
        await page.locator('#shuffleMediaEffects').click();assert.deepEqual(await current(),locked);
        ...
        await page.locator('#shuffleMediaEffects').click();assert.deepEqual(await current(),manual);
        ...
        await page.locator('#mediaAutoPlacement').uncheck();assert.equal(await current(),null);
        await page.locator('#mediaAutoPlacement').check();const restored=await current();assert.ok(restored);
