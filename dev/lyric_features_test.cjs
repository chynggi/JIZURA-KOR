...
(async () => {
  ...
  try {
    for (const locale of ['', 'en/']) {
      ...
      assert.equal(await strongToggle.isChecked(),true);await strongToggle.uncheck();
      assert.equal(await page.evaluate(()=>J.ui.plan.cuts.find(c=>c.line===1).frontmost),false,'automatic frontmost can be overridden');
      ...
      assert.equal(await page.evaluate(()=>J.ui.plan.cuts.find(c=>c.line===1).frontmost),false);
